...
public class PortfolioManagementCtl extends BaseCtl {
	...
	protected boolean validate(HttpServletRequest request) {
		...
		if (DataValidator.isNull(request.getParameter("portfolioName"))) {
			...
			request.setAttribute("portfolioName", "portfolioName must be alphanumeric");
		} 
		 else if (request.getParameter("portfolioName").length() > 30) {
				request.setAttribute("portfolioName", "portfolioName contain 30 characters");
				pass = false;
			}  else if ( 3  > request.getParameter("portfolioName").length()) {
		...
		if (DataValidator.isNull(request.getParameter("initialInvestmentAmount"))) {
			...
			request.setAttribute("initialInvestmentAmount", "initialInvestmentAmount must be numeric");

		} 
		else if (!DataValidator.isPositveNumber(Integer.parseInt(request.getParameter("initialInvestmentAmount")))) {
			request.setAttribute("initialInvestmentAmount", "initialInvestmentAmount must contain positive number");
		...
		if (DataValidator.isNull(request.getParameter("investmentStrategy"))) {
			...
			pass = false;
		} else if (request.getParameter("investmentStrategy").length() > 200) {
			request.setAttribute("investmentStrategy", "investmentStrategy contain 200 characters");
			pass = false;
		} else if ( 10  > request.getParameter("investmentStrategy").length()) {
			request.setAttribute("investmentStrategy", "investmentStrategy  must be at least 10 characters long.");
			pass = false;
			return pass;
